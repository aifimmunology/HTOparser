...
print(c(
    ...
    paste0("IN  HTO barcode key: ", in_key),
...
Generate singlet counts for each hash
```{r Count Singlets Per Hash}
singlet_summary <- make_singlet_summary(category_results$hash_category_table,
                                        valid_htos = hto_ref$hto_barcode)
```

```{r Display Singlet Counts, echo = FALSE, results = 'asis'}
rownames(singlet_summary) <- NULL
knitr::kable(singlet_summary, caption = "Singlet Hash Count Summary")
```
...
ggplot() +
  ...
  scale_x_continuous("",
                     ...
                     labels = singlet_plot_data$hto_barcode) +
